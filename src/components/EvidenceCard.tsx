    ...
    <section className="pathsplit-card relative overflow-hidden">
      ...
      <div className="relative space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <div className="pathsplit-section-kicker">平行人生证据卡</div>
          ...
          {card.paths.map((path, index) => (
            <article key={path.agentId} className="pathsplit-evidence-path">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="text-right text-xs uppercase tracking-[0.22em] text-stone-500">{path.label}</div>
              </div>
              <p className="mt-4 text-sm leading-7 text-stone-800">{path.keyDecision}</p>
              <p className="mt-3 text-sm leading-7 text-stone-600">{path.outcome}</p>
              ...
              <div className="mt-5 border-t border-black/8 pt-4 text-sm leading-7 text-stone-700">
                <span className="mr-2 text-xs uppercase tracking-[0.22em] text-stone-500">如果重来</span>
                {path.ifAgain}
              </div>
            </article>
          ))}
